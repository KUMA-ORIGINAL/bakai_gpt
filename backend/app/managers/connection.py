...
class ConnectionManager:
    broadcaster = Broadcast("redis://redis:6379/0")
    ...
    def __init__(self):
        ...
        Something like:
        {
            channel_id: {user1, user2, user3},
            channel_id_1: {user1, user3}
        }
        """
    ...
    async def add_user_connection_to_room(self, channel_id: str, user_id: str) -> (bool, str):
        """
        ...
        if channel_id not in self.connections.keys():
            self.connections[channel_id] = {user_id}
            ...
            subscribe_n_listen_task = asyncio.create_task(self._subscribe_and_listen_to_channel(channel_id=channel_id))
            ...
            self.connections[channel_id].add(user_id)
    ...
    async def _consume_events(self, message: MessageToChannelSchema):
        """
        Function to consume a message and send to all connect clients
        """

        room_connections = self.connections.get(message.channel_id, {})
        ...
        for connection in users_ws_connections:
            is_sent, sent_message_response_info = await self._send_message_to_ws_connection(
                message=f"Room {message.channel_id} --> {message.message}", ws_connection=connection)


    async def _subscribe_and_listen_to_channel(self, channel_id: str):
        """
        This function subscribes to a channel and listens to event in the channel
        """

        async with self.broadcaster.subscribe(channel=channel_id) as subscriber:
            """
            ...
            async for event in subscriber:
                print(event.message)
                message = MessageToChannelSchema.model_validate_json(event.message)

                await self._consume_events(message=message)

    async def send_message_to_room(self, message: MessageToChannelSchema):
        ...
        await self.broadcaster.publish(channel=message.channel_id, message=message.model_dump_json())
